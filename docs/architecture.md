...
Trajectory screenshots використовують initial-pose-aligned estimates: reference
задає лише початкову точку та напрям за першим стабільним відрізком. Подальший
шлях не використовується для підлаштування estimate. Відстань між кольоровою
estimate та пунктирною VRS-GPS reference показує accumulated drift, а легенда
наводить Initial-pose RMSE. Global SE(2) ATE залишається окремою табличною
метрикою форми траєкторії після offline alignment за всіма matched points.
