...
траєкторією. Кольорова лінія є оцінкою estimator після Global SE(2) alignment:
до неї один раз застосовано поворот і зміщення, знайдені за всіма доступними
reference точками. Масштаб траєкторії не змінюється. Тому число `ATE` у легенді
відповідає видимій відстані між лініями. Це offline evaluation, а не поправка,
доступна estimator під час руху.

Initial-pose alignment використовується в окремій колонці таблиць і на RMSE
bar chart. Він суміщає тільки старт і початковий напрям, тому краще показує
помилку, яка накопичується від старту без post-processing за всією reference
траєкторією.
