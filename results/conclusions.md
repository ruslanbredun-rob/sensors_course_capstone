...
траєкторію. Кольорова estimate траєкторія показана після initial-pose alignment:
суміщаються лише стартова точка та початковий напрям за першим стабільним
відрізком руху. Подальша reference траєкторія не використовується для
підлаштування estimate, scale не змінюється. Тому графіки показують фактично
накопичений drift, а в легенді наведено `Initial-pose RMSE`.

Global SE(2) ATE у таблицях є додатковою оцінкою форми після offline повороту
та зміщення за всією траєкторією. Наприклад, `urban35 VIO` має Global ATE
4.63 м, але Initial-pose RMSE 60.37 м. Для оцінки автономного проходження
маршруту більш показовими є initial-pose графік і Initial-pose RMSE.
